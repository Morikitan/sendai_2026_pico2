...
constexpr int RX_LINE_SIZE = 128;
...
ObjData red_objs[MAX_OBJECTS];
ObjData blue_objs[MAX_OBJECTS];
ObjData can_objs[MAX_OBJECTS];
...
void StoreRed(int index, float angle, int area)
{
    if (red_count >= MAX_OBJECTS)
        return;
    red_objs[red_count++] = {index, angle, area, 'N'};
}

void StoreBlue(int index, float angle, int area)
{
    if (blue_count >= MAX_OBJECTS)
        return;
    blue_objs[blue_count++] = {index, angle, area, 'N'};
}

void StoreCan(int index, float angle, char orient, int area)
{
    if (can_count >= MAX_OBJECTS)
        return;
