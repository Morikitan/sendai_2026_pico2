...
int speed;
uint32_t firstTime;
bool isCatchCan = false;
int redBallNumber = 0;
int blueBallNumber = 0;
int canNumber = 0;
bool isNotPlusTask = false;
...
void MainMove(){
    if(task == 0 || task == 1){ //ライン上まで移動
        ...
        while(objectNumber < 5){
            ...
            if(objectNumber == 5){
                firstTime = time_us_32();
                while((time_us_32() - firstTime) / 1000 < 250){
                    PrintDisplayMode();
        ...
        bool isCatchCan = false;
        while(objectNumber < preObjectNumber + 2){
            uint8_t cmd = 0x11;
            uart_write_blocking(camera_uart,&cmd,1);
            sleep_ms(1);
            objID = RotationToObject(false);
    ...
    }else if(task <= 18){//ライントレース
        NewLineTrace();
    }else if(task <= 19){//横缶を取る
        MainMotorState(0,0);
        gpio_put(buzzer_pin,1);
        sleep_ms(500);
        gpio_put(buzzer_pin,0);
        ...
        preLineAngle = 180;
        task++;
    }else if(task <= 22){//後ろライントレース
        BackLineTrace();
    }else if(task <= 23){//横缶を排出する
        gpio_put(buzzer_pin,0);
        TrashHorizonCan();
        firstTime = time_us_32();
        while((time_us_32() - firstTime) / 1000 < 1000){
            StraightLineTrace(0,40);
        }
        task++;
    }else if(task <= 25){//ライントレース
        NewLineTrace();
    }else if((time_us_32() - allFirstTime) / 1000000 < 480 && objectNumber < 11){//残りをすべて回収する
        MainMotorState(0,0);
        sleep_ms(250);
        MainMotorState(0,0);
        sleep_ms(250);
        ...
        int objID;
        isCatchCan = false;
        while(redBallNumber < 3 && objectNumber < 11 && (time_us_32() - allFirstTime) / 1000000 < 510){
            // uint8_t cmd = 0x11;
            // uart_write_blocking(camera_uart,&cmd,1);
            sleep_ms(1);
            objID = RotationToObject(false);
        ...
        if(redBallNumber < 3 && objectNumber < 11 && (time_us_32() - allFirstTime) / 1000000 < 450){
            lastLineTime = time_us_32() / 1000;
            preTask = task;
            while(task < preTask + 2){
                if(task == preTask) firstTime = time_us_32();
                do{
                    PrintDisplayMode();
                    GetDataFromLineToMain();
                    GetGyroAngleFromSub();
...
void BackLineTrace(){
    ...
    if(270 < circleLineAngle && circleLineAngle < 360){
        //右に曲がる
        MainMotorState((int)(3.125 * speed),-(int)(12.5 * speed));
    }else if(0 < circleLineAngle && circleLineAngle < 90){
        //左に曲がる
        MainMotorState(-(int)(12.5 * speed),(int)(3.125 * speed));
...
void BackToLine(){
    GetGyroAngleFromSub();
    int backAngle;
    if(catchObjectDistance > 450000){
        firstTime = time_us_32() / 1000;
        while(time_us_32() / 1000 < firstTime + 500){
            PrintDisplayMode();
            GetGyroAngleFromSub();
    ...
    if(catchObjectDistance < 350000){
        //進まな過ぎて線を検知できないとき
        if(0 <= angleX && angleX < 180){
            //左側にいる
            RotationToAngle(180);
            MainMotorState(100,100);
            sleep_ms(3500u - catchObjectDistance / 100);
            RotationToAngle(90);
            backAngle = 90;
        }else {
            //右側にいる
        ...
        firstTime = time_us_32();
        uint32_t nowTime = time_us_32() / 1000;
        while(nowTime + 1000 > time_us_32() / 1000){
            PrintDisplayMode();
            GetGyroAngleFromSub();
            GetDataFromLineToMain();
            circleLineAngle = GetCircleLineVector(20,true,true);
        ...
        if(catchObjectDistance > 450000){
            ...
            firstTime = time_us_32();
            uint32_t nowTime = time_us_32() / 1000;
            while(nowTime + 500 > time_us_32() / 1000){
                PrintDisplayMode();
                GetGyroAngleFromSub();
                GetDataFromLineToMain();
                circleLineAngle = GetCircleLineVector(20,true,true);
...
void CatchBall(){
    ...
    do{
        ...
        GetGyroAngleFromSub();
        now = time_us_32();
        if(!(d == 999 && (preX > centerX + 5 || preX < centerX - 5)))catchObjectDistance += 100 * sin((angleX - 90) / 180.0 * 3.1416) * (now - prenow) / 1000.0;
        prenow = now;
    ...
    SetServoAngleFromMain(servo_left_claw_pin,130);
    SetServoAngleFromMain(servo_right_claw_pin,50);
    sleep_ms(1000);
...
void CatchHorizonCan(){
    ...
    do{
        ...
        if(preX > centerX + 5){
            MainMotorState(90,110);
        }else if(preX < centerX - 5){
            MainMotorState(110,90);
        }else{
            MainMotorState(100,100);
        }
        GetDistanceFromSub();
        ...
        if(cameraInformation[number].y < 105){
            GetGyroAngleFromSub();
            if(angleX > 180){
    ...
    MainMotorState(0,0);
    GetCurrentFromSub();
    int standardCurrent = current[0] + current[1];
    SetServoAngleFromMain(servo_left_claw_pin,20);
    SetServoAngleFromMain(servo_right_claw_pin,160);
    sleep_ms(600);
    int servoAngle = 30;
    while(servoAngle <= 160 && current[0] + current[1] > standardCurrent + 1000){
        PrintDisplayMode();
    ...
    SetServoAngleFromMain(servo_right_claw_pin,220 - servoAngle);
    MainMotorState(0,0);
    if(servoAngle > 20) isCatchHorizonCan = true;
...
void CatchPetBottle(){
    ...
    firstTime = time_us_32() / 1000;
    while(time_us_32() / 1000 < firstTime + 1000){
        PrintDisplayMode();
        GetGyroAngleFromSub();
        if((time_us_32() - firstTime) / 2000 > 100){
            MainMotorState(100 - (int)((angleX - 180) * 10),100 + (int)((angleX - 180) * 10));
...
void NewLineTrace(){
    ...
    if(90 < circleLineAngle && circleLineAngle < 180){
        //右に曲がる
        MainMotorState((int)(12.5 * speed),-(int)(2.21 * speed));
    }else if(180 < circleLineAngle && circleLineAngle < 270){
        //左に曲がる
        MainMotorState(-(int)(2.21 * speed),(int)(12.5 * speed));
    }else if(VectorAbsoluteValue > 0.4){
...
void OnWall(int angle){
    ...
    while(!gpio_get(touch_sensor_back_left_pin) || !gpio_get(touch_sensor_back_right_pin)){
        PrintDisplayMode();
        GetGyroAngleFromSub();
        if(angleX > 180 && angle == 0) angleX -= 360; 
        if((time_us_32() - firstTime) / 2000 > 200){
            MainMotorState(- 200 - (int)((angleX - angle) * 10),- 200 + (int)((angleX - angle) * 10));
        }else{
...
void PassTheSpace(){
    firstTime = time_us_32();
    while((time_us_32() - firstTime) / 1000 < 600){
        PrintDisplayMode();
        GetGyroAngleFromSub();
        GetDataFromLineToMain();
        if(angleX > 180) angleX -= 360;
    ...
    sleep_ms(250);
    firstTime = time_us_32();
    while((time_us_32() - firstTime) / 1000 < 1300){
        PrintDisplayMode();
        GetDataFromLineToMain();
        GetGyroAngleFromSub();
        if((time_us_32() - firstTime) / 2000 > 400){
...
int RotationToObject(bool isHorizonCan){
    ...
    retireNumber = 0;
    while(((time_us_32() - rotationTime) / 1000 < 500 && isHorizonCan) || ((time_us_32() - rotationTime) / 1000 < 500 && !isHorizonCan) || retireNumber != 0){
        PrintDisplayMode();
        int num_objects = 0;
        num_objects = UseCamera();
        ...
        if((rotationTime - firstRotationTime)/1000 > 10000){
            return 999;
        } 
        SendBufferToDisplay();
    }
    ...
    while(true){
        ...
        for(int i = 0;i < num_objects;i++){
            if(Y > cameraInformation[i].y && 3 <= cameraInformation[i].obj_id && cameraInformation[i].obj_id <= 6){
                if(((isGreaterThancenterX == 1 && cameraInformation[i].x > centerX - 5) || (isGreaterThancenterX == 2 && cameraInformation[i].x < centerX + 5)) && (cameraInformation[i].obj_id == decidedObj_id || (cameraInformation[i].obj_id == 5 && decidedObj_id == 6) || (cameraInformation[i].obj_id == 6 && decidedObj_id == 5))){
                    if((firstTime / 1000 + 5000) > time_us_32() / 1000 || cameraInformation[i].x > centerX){
                        number = i;
                        Y = cameraInformation[i].y;
                    }
                }
        ...
        if(Y == 999 || num_objects == 0){
            MainMotorState(0,0);
            if(serialWatch == "oth") WriteTextOnDisplay(5,30,"999",12,false,false);
            SendBufferToDisplay();
            gotoNumber++;
            if(gotoNumber > 50) goto Find;
            continue;
        }
        //カメラの画像は上下左右反転していることに注意
        if(cameraInformation[number].x > centerX){
            //左側にある　→　反時計回りに回転
            if(gpio_get(touch_sensor_back_left_pin)){
                //左壁に当たっている
            ...
            //右側にある　→　時計回りに回転
            if(gpio_get(touch_sensor_back_right_pin)){
                //右壁に当たっている
                if((cameraInformation[number].x - centerX) * 2 > 400){
                    ...
                    MainMotorState((cameraInformation[number].x - centerX) * 2,0);
                }
            }else{
                //右壁に当たってない
                if((cameraInformation[number].x - centerX) * 2 > 400){
...
void StraightLineTrace(int angle,int speed2){
    ...
    if(90 < circleLineAngle && circleLineAngle < 180 && (angleX - angle < -5 && angle != 0 || (180 < angleX && angleX < 355 && angle == 0))){
        //右に曲がる
        MainMotorState((int)(12.5 * speed2),-(int)(6.25 * speed2));
    }else if(180 < circleLineAngle && circleLineAngle < 270 && angleX - angle > 5){
        //左に曲がる
        MainMotorState(-(int)(6.25 * speed2),(int)(12.5 * speed2));
    }else if(VectorAbsoluteValue > 0.4){
        if((85 < circleLineAngle && circleLineAngle < 90) || 285 < circleLineAngle ){
            //右に曲がる
            MainMotorState((int)(6.25 * speed2),-(int)(6.25 * speed2));
        }else if(circleLineAngle < 75 || (270 < circleLineAngle && circleLineAngle < 275)){
            //左に曲がる
            MainMotorState(-(int)(6.25 * speed2),(int)(6.25 * speed2));
        }else{
