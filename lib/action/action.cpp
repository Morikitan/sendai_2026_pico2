...
float horizonObjectDistance;//横方向への距離
...
//3 : それ以外
int objectTask;
bool isMissPetBottle = false;//ペットボトルを逃したか
...
void MainMove(){
    if(task == 0 || task == 1){ //ライン上まで移動
        ...
        if(isYosen){
            ...
            GetGyroAngleFromSub();
            RotationToAngle(0);
            DaikeiKasokuLoop(850,400,0);
            pLeft = 400;pRight = 400;
            firstTime = time_us_32();
        }
        ...
        int objID = 555;
        isCatchHorizonCan = false;
        while((objectTask == 1 || objectTask == 2 || (objectTask == 3 && (redBallNumber < 3 || allRedBallNumber == 6) && (canNumber < 2 || allCanNumber == 4))) && objectNumber < 12 && (time_us_32() - allFirstTime) / 1000000 < (510 - startTime) && !isCatchHorizonCan){
            // uint8_t cmd = 0x11;
        ...
        if(canNumber == 0 && !isCatchHorizonCan && redBallNumber == 0 && (time_us_32() - allFirstTime) / 1000000 < (480 - startTime)){
            //すぐに戻る
            preLineAngle = 90;
            lineAngle = 90;
            lastLineTime = time_us_32() / 1000;
            ...
            if(redBallNumber == 0 && (time_us_32() - allFirstTime) / 1000000 < (480 - startTime)){
                //すぐに戻る
                preLineAngle = 0;
                lineAngle = 0;
                lastLineTime = time_us_32() / 1000;
                ...
                RotationToAngle(0);
                if(objectNumber < 12 && (time_us_32() - allFirstTime) / 1000000 < (480 - startTime)){
                    //すぐに戻る
                    pLeft = 0;pRight = 0;
                    preLineAngle = 0;
                    lineAngle = 0;
...
void CatchBall(bool isSuction){
    ...
    do{
        ...
        if(d == 999 && (preX > centerX + 5 || preX < centerX - 5)){
            ...
            prenow = now;
            GetDistanceFromSub();
            if((distance > 260 && task < 7) || (distance > 270 && task > 7)){
                tofTime = time_us_32();
            }
            SendBufferToDisplay();
        ...
        if(distance == 0xFFFF){
            ...
            if(!(d == 999 && (preX > centerX + 5 || preX < centerX - 5))){
                catchObjectDistance += s * sin((angleX - 90) / 180.0 * 3.1416) * (now - prenow) / 1000.0;
                horizonObjectDistance += s * cos((angleX - 90) / 180.0 * 3.1416) * (now - prenow) / 1000.0;
            }
            prenow = now;
        }
        if((distance > 260 && task < 7) || (distance > 270 && task > 7)){
    ...
    MainMotorState(0,0);
    PrintDisplayMode();
    if(horizonObjectDistance > 200000 || horizonObjectDistance < -200000){
        //壁際
        GetGyroAngleFromSub();
        if(angleX > 180){
            RotationToAngle((int)(angleX - 10));
    ...
    sleep_ms(250);
    if(isSuction && allRedBallNumber <= 2 && redBallNumber == 0 && task > 7){
        //ピラミッドを崩すとき
    ...
    while(true){
        ...
        if((colorTime + 1000000 < time_us_32() && isSuction) || (colorTime + 5000000 < time_us_32() && !isSuction)){
            ...
            sleep_ms(500);
            SetServoAngleFromMain(servo_arm_up_and_down_pin,140);
            sleep_ms(1500);
...
void CatchCan(){
    ...
    if(cameraInformation[number].obj_id == 5){
        //中心線より左側
        SetServoAngleFromMain(servo_left_claw_pin,100);
        SetServoAngleFromMain(servo_right_claw_pin,80);
    }else{
    ...
    if((angleX < 157 || angleX > 203) && task < 7){
        DaikeiKasokuLoop(1500,100,999); //1250 * 100
        SetSuctionMotorSpeedFromMain(150);
        sleep_ms(1000);
        DaikeiKasokuLoop(900,-100,999); //1250 * 100
        SetSuctionMotorSpeedFromMain(0);
        sleep_ms(1000);
        catchObjectDistance += 600 * 100 * sin((angleX - 90) / 180.0 * 3.1416);
    }else if(task < 7){
        catchObjectDistance += 1650 * 100 * sin((angleX - 90) / 180.0 * 3.1416);
        DaikeiKasokuLoop(1750,100,999); //1250 * 100
    }else{
        catchObjectDistance += 1650 * 100 * sin((angleX - 90) / 180.0 * 3.1416);
    ...
    GetCurrentFromSub();
    int standardCurrent[2] = {current[0],current[1]};
    while(servoAngle <= 140 && (current[0] > standardCurrent[0] * 0.8 && current[1] > standardCurrent[1] * 0.8)){
        PrintDisplayMode();
        servoAngle += 5;
        SetServoAngleFromMain(servo_left_claw_pin,servoAngle);
        SetServoAngleFromMain(servo_right_claw_pin,180 - servoAngle);
    ...
    if(63 < servoAngle && servoAngle < 130 && !(isGoto && task < 7)){
        if(task < 7){
            ...
            catchObjectDistance -= 1900 * 100 * sin((angleX - 90) / 180.0 * 3.1416);
            SetSuctionMotorSpeedFromMain(0);
            sleep_ms(100);
            isGoto = true;
            goto CanCatch;
        ...
        if(!isGoto && task > 7){
            DaikeiKasokuLoop(500,-200,999);
            catchObjectDistance -= 60000 * sin((angleX - 90) / 180.0 * 3.1416);
            isGoto = true;
            goto CanCatch;
        }
        if(task < 7){
...
void CatchPetBottle(){
    ...
    while(deltaTime < 50){
        ...
        preTime = time_us_32() / 1000;
        SendBufferToDisplay();
        if(((time_us_32() - firstTime) / 1000 > 6000 && objectNumber < 5) || ((time_us_32() - firstTime) / 1000 > 4000 && objectNumber >= 5)){
            DaikeiKasokuLoop(1500,-150,180);
            isBreak = true;
            break;
        } 
    }
    ...
    SetServoAngleFromMain(servo_arm_up_and_down_pin,170);
    if(isBreak) DaikeiKasokuLoop(1500,150,180);
    GetCurrentFromSub();
    if(current[2] > 1000){
        //乗り上げている
        DaikeiKasokuLoop(2000,-100,180);
    }
    uint16_t standardCurrent = current[0] + current[1];
    sleep_ms(1000);
    firstTime = time_us_32();
    MainMotorState(0,0);
    while((time_us_32() - firstTime) / 1000 < 2450){
        GetGyroAngleFromSub();
        if((time_us_32() - firstTime) / 15000 > 200){
            MainMotorState(200 - (int)((angleX-180.0) * 10),200 + (int)((angleX-180.0) * 10));
        }else{
            if(speed > 0){
                MainMotorState((int)(time_us_32() - firstTime) / 15000 - (int)((angleX-180.0) * 10),(int)(time_us_32() - firstTime) / 15000 + (int)((angleX-180.0) * 10));
            }else{
                MainMotorState((int)(time_us_32() - firstTime) / -15000 - (int)((angleX-180.0) * 10),(int)(time_us_32() - firstTime) / -15000 + (int)((angleX-180.0) * 10));
            }  
        }
    ...
    sleep_ms(850);
    GetCurrentFromSub();
    if(current[0] + current[1] < standardCurrent * 1.5 && !isMissPetBottle){
        //缶を取れなかった
        isMissPetBottle = true;
        goto petBottle;//ペットボトルの位置をtofで探すところへ戻る。
    }   
    DaikeiKasokuLoop(1500,-150,180);
    sleep_ms(500);
    SetServoAngleFromMain(servo_left_claw_pin,60);
    SetServoAngleFromMain(servo_right_claw_pin,120);
    sleep_ms(200);
    DaikeiKasokuLoop(200,-100,180);
    SetServoAngleFromMain(servo_arm_up_and_down_pin,163);
    sleep_ms(200);
    DaikeiKasokuLoop(150,100,180);
    SetServoAngleFromMain(servo_left_claw_pin,40);
    SetServoAngleFromMain(servo_right_claw_pin,140);
    sleep_ms(200);
    ...
    SetServoAngleFromMain(servo_arm_left_and_right_pin,90);
    sleep_ms(1000);
    SetServoAngleFromMain(servo_arm_up_and_down_pin,65);
    //指定の場所まで移動
    RotationToAngle(270);
    sleep_ms(500);
    firstTime = time_us_32();
...
int RotationToObject(){
    ...
    int firstFirstRotationTime;
    int retireNumber;
    firstFirstRotationTime = time_us_32();
    Find:
    ...
    while((time_us_32() - rotationTime) / 1000 < 500 || retireNumber != 0){
        ...
        if((time_us_32() - firstFirstRotationTime) / 1000 > 10000 || (rotationTime - firstRotationTime) / 1000 > 9000 || ((rotationTime - firstRotationTime) / 1000 > 4500 && num_objects == 1)){
            // for(int i = 0;i < num_objects;i++){
...
void TrashfromBasket(int object){
    ...
    while((((!circleLineSensor[9] && !circleLineSensor[10] && !circleLineSensor[11]) && object != 2) || ((!circleLineSensor[7] || !circleLineSensor[12]) && object == 2)) || (time_us_32() - firstTime) / 1000 < 1250){
        ...
        DaikeiKasoku(-300,90);
        SendBufferToDisplay();
        if(((time_us_32() - firstTime) / 1000 > 6000 && object == 2) || ((time_us_32() - firstTime) / 1000 > 3000 && (object == 1 || object == 3))){
            break;
        }
    }
    MainMotorState(0,0);
    if(object == 1){
        if(objectTask == 1 && allRedBallNumber >= 3){
            DaikeiKasokuLoop(500,250,90);
            if(allRedBallNumber - redBallNumber < 3){
                //赤ボールのかごのほうに赤ボールが入っている(3つ目まで)
                RotationToAngle(75);
                TrashfromBasketFromMain(10);
            }
            //4～6つ目の赤ボールを捨てる
            RotationToAngle(105);
            TrashfromBasketFromMain(2);
        }else{
            DaikeiKasokuLoop(500,250,90);
            RotationToAngle(75);
            TrashfromBasketFromMain(10);
